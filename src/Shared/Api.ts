...
export const getApiPosts = (status?: PostStatus, options?: FetchArgsOptions):
  Promise<GetApiPostsFetchResponse> => {
    const queryParams = {
      "status": status
    }
    return apiGet(`${getApiUrl()}${getApiPostsPath()}`, options, queryParams) as Promise<GetApiPostsFetchResponse>;
}
...
export type PutApiPostsIdModerateFetchResponse = 
| FetchResponse<void, 200> 
| ErrorResponse;

export const putApiPostsIdModeratePath = (id: number) => `/api/Posts/${id}/moderate`;

export const putApiPostsIdModerate = (requestContract: ModeratePostDTO, id: number, options?: FetchArgsOptions):
  Promise<PutApiPostsIdModerateFetchResponse> => {
    const requestData = getApiRequestData<ModeratePostDTO>(requestContract, false);

    return apiPut(`${getApiUrl()}${putApiPostsIdModeratePath(id)}`, requestData, options) as Promise<PutApiPostsIdModerateFetchResponse>;
}
