...
export class ApiCallError extends Error {
  data: FetchResponseOfError | FetchResponse<Statuses400s>;
  queryInfo?: QueryInfo;
...
export const processResponseQuery =
  <TResponse extends BaseResponse, TQueryKey extends QueryKey = QueryKey>(
    request: (
      args: QueryFunctionContext<TQueryKey>,
      headers?: Headers
    ) => Promise<TResponse>,
    options?: ProcessResponseOptions
  ) =>
  async (
    args: QueryFunctionContext<TQueryKey>
  ): Promise<SuccessResponseData<TResponse>> => {
    const response = await request(args);
...
export const processResponseAndExtractData =
  <
    ...
    TExtract extends keyof SuccessResponseData<TResponse>
  >(
    request: (args: TArgs, headers?: Headers) => Promise<TResponse>,
...
function processResponseStatusCodeBase<
  TResponse extends BaseResponse,
...
>(
  ...
  if (response?.status >= 400 || response?.status === 0) {
    ...
    throw new ApiCallError(
      message,
      response as FetchResponseOfError,
      options,
      args?.queryKey,
      args?.meta
    );
...
function processResponseStatusCode<
  TResponse extends BaseResponse,
  TQueryKey extends QueryKey = QueryKey
>(
  response: TResponse,
